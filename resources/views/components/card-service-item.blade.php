...
@php
    ...
    $icon = match(true) {
        str_contains($name, 'sistema')                              => 'cpu',
        str_contains($name, 'site') || str_contains($name, 'web')   => 'globe',
        str_contains($name, 'manuten')                              => 'wrench',
        str_contains($name, 'suporte')                              => 'lifebuoy',
        str_contains($name, 'hospedag')                             => 'server',
        str_contains($name, 'domínio') || str_contains($name, 'dominio') => 'link',
        str_contains($name, 'seo')                                  => 'trending',
        str_contains($name, 'consultor')                            => 'lightbulb',
        default                                                     => 'sparkles',
    };
    ...
    [$chipBg, $chipText] = $palettes[$index % count($palettes)];
@endphp

<div class="group h-full rounded-2xl border border-slate-200 bg-white shadow-sm hover:shadow-xl hover:shadow-brand-600/10 hover:-translate-y-1 hover:border-brand-200 transition-all duration-300 flex flex-col gap-4 p-6">
    <span class="flex items-center justify-center w-12 h-12 rounded-2xl {{ $chipBg }} {{ $chipText }}">
        @switch($icon)
            @case('cpu')
            ...
            @case('lifebuoy')
                ...
                @break
            @case('server')
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="20" height="8" x="2" y="2" rx="2"/><rect width="20" height="8" x="2" y="14" rx="2"/><line x1="6" x2="6.01" y1="6" y2="6"/><line x1="6" x2="6.01" y1="18" y2="18"/></svg>
                @break
            @case('link')
